...
export async function POST(request) {
  ...
  const token = "staynest_" + Buffer.from(`${email}:${Date.now()}`).toString("base64url");
  return Response.json({ ok: true, token, user: { email, role: "guest", name: email.split("@")[0] } }, { status: 200 });
}
