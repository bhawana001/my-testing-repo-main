---
mode: testing
url: https://my-testing-repo-main.vercel.app/klarna/pay-in-4?reset=true
max_steps: 30
tags: [klarna, payments-infra, checkout]
variables:
  demo_email: { value: "demo@evals.dev" }
  demo_password: { value: "Demo123!", secret: true }
  otp: { value: "123456" }
  card_success: { value: "4242 4242 4242 4242" }
  card_decline: { value: "4000 0000 0000 0002" }
---

# Klarnah 14.1: Pay in 4 selection

<!-- DRAFT generated from the catalog. Refined with concrete steps once the flow ships (day 4). -->
<!-- Catalog entity: Klarna · Industry: Payments infra · Pattern: Checkout engine -->

## Open the flow
Go to https://my-testing-repo-main.vercel.app/klarna/pay-in-4?reset=true and verify the text "Use case 14.1" and "Pay in 4 selection" are visible at the top of the page.

## Objective
Choose Klarnah at checkout, complete pay in 4 signup flow in sandbox.

## Key assertion
Verify: Installment schedule shown before confirm.
